...
export class S5ClassworkService {
  rentalList: { count: number; titles: string } = {
    count: 3,
    titles: 'The Room, Cats, Catwoman',
  };

  getRentalList(): Observable<{ count: number; titles: string }> {
    return of(this.rentalList);
  }

  addRental(title: string) {
    this.rentalList.titles += ', ' + title;
    this.rentalList.count++;
  }

  getFlop(id: number) {
    return this.getFlops().pipe(
      map((flops) => {
  ...
  getFlopsMaxStars(stars: number) {
    return this.getFlops().pipe(
      map((flops) => {
        return flops.filter((movies) => movies.stars <= stars);
      })
    );
  }
